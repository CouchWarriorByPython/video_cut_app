...
def update_video_statistics(source_video_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Get all clips for this source video
        clips = clip_repo.find_all(filter_query={"source_video_id": source_video_id})
        ...
        # Get unique ML projects
        ml_projects = list(set(c.get("ml_project") for c in clips))

        # Calculate total duration of clips
        total_duration = sum(c.get("duration_sec", 0) for c in clips)

        # Update source video with statistics
        update_data = {
            "clips": [c["_id"] for c in clips],
            "video_attributes": {
                **source_repo.find_by_id(source_video_id).get("video_attributes", {}),
                "total_clips": total_clips,
                "processed_clips": processed_clips,
                "failed_clips": failed_clips,
                "ml_projects": ml_projects,
                "total_clips_duration_sec": total_duration
            },
            "updated_at_utc": source_repo.find_by_id(source_video_id)["updated_at_utc"]  # Keep current timestamp
        }
